...
#%%
nine = ['341I', '356T', '365F', '367A', '386E', '428N', '453F', '460K', '484K']
nine_iter = list(itertools.permutations(nine, 2))

seqs = []
for i in nine_iter:
    delta_seq_tomut = list(variant_seqs.iloc[5,0])
    for j in i:
        mut = list(j)
        add = mut[-1]
        mut.pop(-1)
        res = int(''.join(mut)) - 331
        delta_seq_tomut[res] = add
    seqs.append([''.join(delta_seq_tomut), i])
seqs = pd.DataFrame(seqs)
seqs.drop_duplicates(subset = 0, inplace = True)

seqs_ohe = ohe_encode(seqs)
ace_seqs = pd.DataFrame(ace_ridge.predict(seqs_ohe))
pAb_seqs = pd.DataFrame(pAb_ridge.predict(seqs_ohe))

plt.scatter(ace_seqs.iloc[:,0], pAb_seqs.iloc[:,0]*100, s = 125, c = lighten_color(variant_colors[5], 0.15), edgecolor = 'k', linewidth = 0.25)
plt.scatter(ace_variant[5], pAb_variant[5]*100, c = variant_colors[5], s = 230, edgecolor = 'k', linewidth = 0.25)
plt.xlim(11.6, 12.2)
plt.ylim(4.5, 14)
plt.xticks([11.6, 11.8, 12.0, 12.2], fontsize = 26)
plt.yticks([6,8,10,12,14],fontsize = 26)
